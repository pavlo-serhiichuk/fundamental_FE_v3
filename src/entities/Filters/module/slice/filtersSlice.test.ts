...
describe('filtersSlice', () => {
  test('set search value', () => {
    ...
    expect(
      filtersReducer(state, filtersActions.setSearchValue('value')),
    ).toEqual({ searchValue: 'value' })
  })

  test('set order', () => {
    const state: FiltersSchema = { orderBy: 'asc' }
    expect(filtersReducer(state, filtersActions.setOrderBy('desc'))).toEqual({
      orderBy: 'desc',
    })
  })
  ...
  test('set sort', () => {
    ...
    expect(
      filtersReducer(state, filtersActions.setSortBy(SortByType.NAME)),
